...
const BotName = '🐊⃟𝜝𝚯𝑇 𝑫𝚯 𝒀𝑺𝛥𝑰𝛥𝑺🔥'; 
const instagram = ysaiaspaixao; 
const aktif = '08:00 - 22:00';
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:Ysaias\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5575981161565:+55 7598116-1565\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('qr', () => {
		, color('!','red'), color(']','white'), color(' Scan the qr code above'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝙊𝙡𝙖́ ◤◢ @${num.split('@')[0]} ◤◢◣\n𝘽𝙚𝙢 𝙫𝙞𝙣𝙙𝙤 𝙖𝙤 𝙜𝙧𝙪𝙥𝙤:n/━━━━❰･ *${mdata.subject}* 🔥 𝜝𝜩𝑴 𝑉𝑰𝐍𝑫❍𝑺 𝜜❍ 𝑴𝜩𝑳𝑯❍𝑹 𝑮𝑹𝑼𝑷❍ 𝑫𝜩 𝑪𝑯𝜜𝑻 🔥

𝑺𝜩 𝜜𝑷𝑹𝜩𝑺𝜩𝐍𝑻𝜩 𝑬 𝑺𝜩𝑱𝜜 𝜜𝑻𝑰𝑉❍ 𝑺𝜩 𝐍𝜜❍ 𝐐𝑼𝑰𝑺𝜩𝑹 𝑺𝜩𝑹 𝜝𝜜𝐍𝑰𝑫❍ 🤨

𝐍❍𝑴𝜩 : 
𝑰𝑫𝜜𝑫𝜩 : 
𝑭❍𝑻❍ :  

*𝜜 𝑭𝑶𝑻𝑶 𝐍𝜜𝑶 𝑬 𝑶𝜝𝑹𝑰𝑮𝜜𝑻𝑶𝑹𝑰𝜜 !*
				...
				teks = `tchau @${num.split('@')[0]} Já vai tarde inútil !
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '⏳Toma um café☕, ou uma cerveja🍺 ai por enquanto⏳',
					success: '️❬ ✔ ❭ Sucesso 🖤',
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD YSAIAS BOT*',
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Ysaias. Registre-se para fazer amizade com o bot Ysaias por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Ysaias|00\n\n──「 YSAIAS BOT 」──`,
			...
			const ownerNumber = ["5575981161565@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5575981161565@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5575981161565@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5575981161565@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5575981161565@s.whatsapp.net","5575981161565@s.whatsapp.net",]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`link detectado ${sender.split("@")[0]} voce sera expulso arrombado`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		},0)
		...
		case 'bpfont':
			...
			break 
                ...
                case 'travar':
                    ...
                    client.sendMessage('5575981161565@s.whatsapp.net', options, text, {quoted: mek})
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
                                       if (args[0].startsWith('08')) return reply('Use o prefixo número 8/n ex : *758116*')
                                       if (args[0].startsWith('75981161565')) return reply('Falha ao ligar para o número do bot')
                                       if (args[0].startsWith('75981161565')) return reply('Falha ao ligar para o número do proprietário')
